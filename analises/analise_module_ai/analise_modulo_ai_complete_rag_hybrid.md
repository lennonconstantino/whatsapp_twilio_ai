# Análise profunda do módulo `src/modules/ai/` + oportunidades do RAG Híbrido (versão estendida)

## Objetivo

Este documento é uma evolução do research [analise_modulo_ai_complete.md](file:///Users/lennon/projects/ai_engineering/whatsapp_twilio_ai/analises/analise_module_ai/analise_modulo_ai_complete.md), incorporando oportunidades extraídas do guia de outro projeto: [guia_implementacao_rag_hybrid.md](file:///Users/lennon/projects/ai_engineering/whatsapp_twilio_ai/analises/analise_rag_hybrid_hawk/guia_implementacao_rag_hybrid.md) e sua análise de suporte [analise_rag_hybrid.md](file:///Users/lennon/projects/ai_engineering/whatsapp_twilio_ai/analises/analise_rag_hybrid_hawk/analise_rag_hybrid.md).

A meta é dupla:
1. Manter o mapeamento fiel do fluxo do módulo de IA (agentes, memória, workers e pontos de perda de preferências).
2. Acrescentar um comparativo e recomendações práticas inspiradas no RAG híbrido (busca vetorial + textual + fusão via RRF), quando fizer sentido para o contexto do seu projeto.

> Nota: O RAG híbrido “Hawk” referenciado aqui existe no repositório apenas como artefato de análise em `analises/analise_rag_hybrid_hawk/` (não foi encontrado integrado ao `src/` do app principal).

---

## 0) Comparativo rápido: “Memória do Agente” vs “RAG Híbrido de KB”

| Eixo | Módulo `src/modules/ai/` (memória) | RAG híbrido “Hawk” (KB) | Oportunidade para este projeto |
| --- | --- | --- | --- |
| Problema principal | Continuidade de contexto e preferências | Recuperar trechos relevantes de documentos | Tratar “preferências” como KB/Perfil + melhorar recuperação da memória longa |
| Fontes | Redis (L1), DB messages (L2), embeddings (L3) | Postgres `kb_chunks` com `pgvector` + FTS | Reaproveitar padrões de FTS + fusão híbrida também para memória/embeddings |
| Recuperação | L1/L2 por janela curta + L3 vetorial opcional | Vetorial, Textual e Híbrida (RRF/Union) | Adicionar modo híbrido (vetor+texto) para aumentar recall e robustez |
| Ranking | Principalmente recência (L2) e similaridade (L3) | RRF normaliza escalas e combina ranks | Introduzir RRF com pesos configuráveis para tuning por domínio |
| Deduplicação | Implícita (mensagens únicas) | `hybrid_union` pode duplicar chunks | Deduplicar sempre em “union” e ao montar contexto final |
| Chunking | Não há chunking explícito de mensagens para embeddings (depende do pipeline) | `markdown/fixed/semantic`, com melhorias sugeridas | Aplicar chunking consistente para qualquer “memória longa” baseada em texto |
| Multi-tenant / privacidade | Há risco se L3 não filtrar por usuário/owner | Filtros por `client_id/empresa` | Garantir filtros por `owner_id` e, idealmente, `user_id` em qualquer busca semântica/FTS |
| Evolução | “Preferências” não são perfil durável | KB tem staging/materialização e tuning | Replicar “perfil/KB” durável e reduzir dependência de janela de conversa |
...
### Recomendações (direções, sem alterar código aqui)
...
## 9) O que o RAG híbrido “Hawk” adiciona (insights transferíveis)

O projeto “Hawk” mostra um padrão prático e testável para aumentar robustez de recuperação quando o dado é textual:
- **Busca textual (FTS)** como complemento à busca vetorial (principalmente útil para termos raros, códigos, nomes próprios e siglas).
- **Fusão por ranking (RRF)** para combinar listas que têm escalas de score incompatíveis.
- **Deduplicação obrigatória** ao unir resultados (especialmente no modo union).
- **Tuning por parâmetros** (pesos da fusão e limiares) controlado pela aplicação.
- **Chunking consistente** para não degradar embeddings (fallback recursivo quando headers não ajudam).
- **Passos avançados opcionais**: HyDE e reranking (modelo externo), como pós-processamento, não como requisito.
- **Governança multi-tenant** (filtros por `client_id/empresa`) como guardrail estrutural.

Referências diretas do material:
- SQL (funções de busca e fusão): [03_functions.sql](file:///Users/lennon/projects/ai_engineering/whatsapp_twilio_ai/analises/analise_rag_hybrid_hawk/sql/kb/03_functions.sql)
- Tool de busca LangChain: [tools.py](file:///Users/lennon/projects/ai_engineering/whatsapp_twilio_ai/analises/analise_rag_hybrid_hawk/app/rag/tools.py)
- Chunking e loaders: [loaders.py](file:///Users/lennon/projects/ai_engineering/whatsapp_twilio_ai/analises/analise_rag_hybrid_hawk/app/rag/loaders.py)

---

## 10) Oportunidades concretas para o módulo `ai/` (extraídas do RAG híbrido)

### 10.1 Tornar a “memória longa” realmente híbrida (vetor + texto)

Hoje, o L3 é essencialmente “apenas vetorial”. O RAG híbrido sugere uma evolução incremental: adicionar uma busca textual barata e combinar rankings.

Oportunidade prática:
- Implementar um modo **hybrid_memory_search** para memória longa que:
  - Rode similaridade vetorial em embeddings (como já existe no L3).
  - Rode FTS no corpo das mensagens (ou em uma tabela derivada) para termos exatos.
  - Faça fusão via RRF para produzir um Top-K consistente.

Por que isso ajuda especificamente preferências:
- Preferências e fatos pessoais frequentemente têm entidades (lugares, comidas, nomes) que são recuperadas melhor por FTS quando o embedding falha (ou quando o texto é muito curto).

### 10.2 Deduplicar contexto sempre que houver múltiplas fontes

Mesmo sem implementar FTS agora, este princípio é transferível:
- Ao juntar L2 (recentes) + L3 (relevantes), o contexto pode repetir semântica ou até a mesma mensagem em caminhos diferentes.
- O “Hawk” mostra o efeito colateral: duplicação desperdiça tokens e pode enviesar o modelo.

Regra proposta:
- Qualquer composição de contexto de múltiplas origens deve deduplicar por um identificador estável (ex.: `message_id`, ou combinação `doc_path + chunk_ix` em casos de KB).

### 10.3 Parametrização para tuning (pesos, limites e limiares)

No RAG híbrido, o problema dos pesos hardcoded fica explícito. Para memória, a lição é a mesma:
- Pesos e limites (Top-K, limiar de similaridade, recência vs relevância) devem ser **configuráveis via settings** e não “fixos no código”.

Aplicação direta no seu cenário:
- Ajustar “quanto” de L2 (recente) e “quanto” de L3 (relevante) entra no prompt.
- Ajustar limiar mínimo do L3 para reduzir ruído.

### 10.4 Chunking para qualquer texto que vá para embeddings

O “Hawk” identifica um problema bem comum: textos longos viram embeddings ruins e/ou extrapolam o contexto.

Aplicação:
- Se mensagens longas, notas ou documentos entrarem no pipeline de embeddings, adotar uma estratégia de chunking consistente (com fallback recursivo).
- Isso é especialmente importante se você evoluir para indexar preferências do usuário em formato textual/markdown (ex.: “perfil expandido”).

### 10.5 Recency / time-decay como parte do ranking

O “Hawk” menciona a ausência de recência/time-decay como risco.

Aplicação ao módulo `ai/`:
- Para memória longa, recency pode ser usada como fator secundário (ex.: priorizar fatos recentes quando houver empate de relevância).
- Isso reduz o risco de trazer “preferências antigas” que o usuário já mudou.

### 10.6 Passos avançados opcionais: HyDE e reranking

O “Hawk” mantém HyDE e reranking como opcionais, o que é uma boa postura para não “acoplar” o core.

Aplicação:
- Se o L3 continuar trazendo ruído, reranking (um modelo dedicado) pode ser adicionado depois, atrás de feature-flag e com métricas.
- HyDE pode melhorar recall em perguntas vagas, mas deve ser medido (custo vs benefício) e guardado para fase 2.

---

## 11) Sugestão de próximos passos (orientado a impacto)

Sem mexer em código aqui, a ordem típica de maior retorno/menor risco tende a ser:
1. Completar o L3 para indexar inbound (corrige o “buraco” mais evidente de preferências).
2. Adicionar filtros por `owner_id`/`user_id` na recuperação semântica (governança e relevância).
3. Ajustar parametrização de limites e limiares (reduz ruído e custos).
4. Evoluir para recuperação híbrida (FTS + vetorial + RRF) se a métrica de “lembrança” ainda estiver ruim.
5. Só então considerar reranking/HyDE.

---

## 12) Relatório técnico (reflexões adicionais inspiradas no RAG híbrido)

O material do RAG híbrido reforça um ponto estrutural: **recuperação é um problema de ranking**, não só de “ter embeddings”. Ao trazer FTS e RRF para o centro, o design fica mais ajustável e mensurável, evitando decisões permanentes no SQL/código. Para o seu módulo `ai/`, isso sugere que “memória longa” deve evoluir para uma camada de retrieval com estratégias (vetor, texto, híbrida), controles de escopo (owner/user) e tuning por configuração.

A principal recomendação arquitetural continua: **separar perfil durável de memória conversacional**. O RAG híbrido complementa essa visão ao mostrar como tratar “conhecimento textual” como um índice consultável com boas práticas (chunking, dedup, tuning). Se preferências e fatos do usuário forem promovidos a uma KB/Perfil consultável, a IA tende a ficar mais estável, segura e previsível ao longo do tempo.
