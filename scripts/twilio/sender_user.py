...
def build_webhook_payload_user(
    message: str, media_url: str = None, from_number: str = None
):
    ...
    payload = {
        ...
        "WaId": (
            from_number.replace("+", "")
            if from_number
            else os.environ.get("MY_PHONE_NUMBER", "+5511999999999").replace("+", "")
        ),
...
def send_to_local_webhook(
    message: str, media_url: str = None, from_number: str = None, port: int = 8080
):
...
Exemplos:
  ...
  python sender_user.py "Teste" --from +5511999999999 --media https://picsum.photos/200 --local
    ...
    parser.add_argument(
        "--from", dest="_from", help="Número de onde vamos mandar (ex: +5511999999999)"
    )
    parser.add_argument(
