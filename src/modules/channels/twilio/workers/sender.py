...
def build_webhook_payload(message: str, media_url: str = None, to_number: str = None):
    ...
    payload = {
        ...
        "To": f"whatsapp:{to_number if to_number else os.environ.get('MY_PHONE_NUMBER', '+5511999999999')}",
    ...
    if media_url:
        payload["NumMedia"] = (
            "1"  # Quando o numero de caracteres de mensagens for menor que 160
        )
...
def send_to_local_webhook(
    message: str, media_url: str = None, to_number: str = None, port: int = 8080
):
    """Envia para webhook local (simula Twilio)"""
    url = f"http://localhost:{port}/channels/twilio/v1/webhooks/inbound"
    payload = build_webhook_payload(message, media_url, to_number)
    headers = {"X-API-Key": os.getenv("INTERNAL_API_KEY")}  # ← Sua própria chave
...
def process_local_mode(message: str, media_url: str, port: int, to_number: str):
    ...
    response = send_to_local_webhook(message, media_url, to_number, port)
    if not response:
        return
...
Exemplos:
  ...
  python sender.py "Teste" --to +5511999999999 --media https://picsum.photos/200
    ...
    parser.add_argument(
        "--media", "-m", help="URL da mídia (imagem, áudio, vídeo, PDF)"
    )
    parser.add_argument(
        "--local", action="store_true", help="Enviar para webhook local antes do Twilio"
    )
    parser.add_argument(
        "--port",
        type=int,
    ...
    if args.local:
        ...
        send_via_twilio(message=args.message, to_number=args.to, media_url=args.media)
