...
| **Segurança** | 🔴 Não Conforme | **Ausência total de autenticação/autorização nos endpoints.** Falta validação de permissões (quem pode criar planos?). |
| **Qualidade** | ⚠️ Parcial | Código limpo (PEP 8, Type Hints), mas cobertura de testes unitários na camada de serviço é inexistente. |
| **Performance** | ⚠️ Parcial | Mistura de código síncrono e assíncrono inadequada no WebhookHandlerService (blocking I/O). |
...
1.  **CRÍTICO - Endpoints Públicos:** As rotas `/v1/subscriptions` e `/v1/plans` estão abertas. Um atacante pode criar planos arbitrários ou inscrever usuários indevidamente.
2.  **ALTO - Bloqueio de Event Loop:** O método `WebhookHandlerService.handle_event` é `async`, mas chama `self.subscription_service.create_subscription` que é síncrono (bloqueante). Em alto tráfego de webhooks, isso degradará a performance de toda a aplicação.
...
1.  **Segurança:** Implementar middleware/dependência de autenticação (`VerifyUser`) em `src/modules/billing/api/v1/router.py` ou nos arquivos individuais. Garantir que `owner_id` venha do token e não do body (ou validar match).
2.  **Performance:** Converter métodos de `SubscriptionService`, `PlanService` e repositórios para `async def`. O driver do Supabase (`supabase-py`) suporta chamadas assíncronas via `execute()`? (Verificar se está usando cliente async ou sync. O código atual usa `.execute()`, que geralmente é síncrono no cliente padrão). Se o cliente for síncrono, encapsular em `run_in_executor` ou adotar cliente async.
...
**Nota Final:** 6.0 (Requer atenção imediata em Segurança)
