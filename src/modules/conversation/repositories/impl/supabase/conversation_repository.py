...
class SupabaseConversationRepository(SupabaseRepository[Conversation], ConversationRepository):
    """
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Conversation]:
        ...
        """
        def _create_sync():
            data_copy = {**data} # Avoid modifying original
            data_copy.pop("session_key", None)
            ...
            # Using SupabaseRepository.create explicitly to avoid super() issues in closure if any
            conversation = super(SupabaseConversationRepository, self).create(data_copy)
    ...
    async def find_by_status(
        ...
        def _find():
            try:
                ...
                logger.error(
                    "Error finding conversations by status",
                    owner_id=owner_id,
                    status=status.value,
                    error=str(e)
                )
    ...
    async def update(
        ...
        def _update():
            ...
            # Use super class update
            result = super(SupabaseConversationRepository, self).update(id_value, data, id_column=id_column, current_version=current_version)
    ...
    async def find_idle_conversations(self, idle_minutes: int, limit: int = 100) -> List[Conversation]:
        def _find():
            try:
    ...
    async def cleanup_expired_conversations(self, limit: int = 100) -> int:
        # This calls other async methods, so it can be async naturally
        processed = 0
        candidates = await self.find_expired_conversations(limit)
