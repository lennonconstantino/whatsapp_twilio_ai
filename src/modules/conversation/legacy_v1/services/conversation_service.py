...
from src.modules.conversation.legacy_v1.components.closure_detector import \
    ClosureDetector
...
class ConversationService:
    ...
    def get_or_create_conversation(
        ...
        if settings.toggle.enable_background_tasks:
            self.conversation_repo.cleanup_expired_conversations(
                owner_id=owner_id, channel=channel
            )
        else:
            logger.info(
                "Background tasks are disabled, skipping cleanup", owner_id=owner_id
            )
        ...
        try:
            ...
            logger.warning(
                "Failed to fetch conversation history for linking", error=str(e)
            )
            ...
            new_metadata.update(
                {
                    ...
                    "previous_ended_at": (
                        previous_conv.ended_at.isoformat()
                        if previous_conv.ended_at
                        else None
                    ),
                    "linked_at": datetime.now(timezone.utc).isoformat(),
                }
    ...
    def add_message(
        ...
        try:
                ...
                try:
                    ...
                    if conversation.status == ConversationStatus.IDLE_TIMEOUT.value:
                        ...
                        if not updated:
                            raise ConcurrencyError(
                                "Failed to update status (concurrency)",
                                current_version=0,
                            )
                        ...
                        if not updated:
                            raise ConcurrencyError(
                                "Failed to update context (concurrency)",
                                current_version=0,
                            )
                        conversation = updated
                    ...
                    if conversation.status == ConversationStatus.PENDING.value:
                        ...
                        if message_create.message_owner in [
                            ...
                            if not updated:
                                raise ConcurrencyError(
                                    "Failed to update status", current_version=0
                                )
                            conversation = updated
                            ...
                            if (
                                hasattr(message_create, "user_id")
                                and message_create.user_id
                            ):
                                context["accepted_by"][
                                    "user_id"
                                ] = message_create.user_id
                            ...
                            if not updated:
                                raise ConcurrencyError(
                                    "Failed to update context", current_version=0
                                )
                            conversation = updated
                        else:
                    ...
                    is_transition_error = isinstance(
                        e, ValueError
                    ) and "transition" in str(e)
                    ...
                    if attempt == max_retries - 1:
                        logger.error(
                            "Max retries reached for add_message state transition",
                            conv_id=conversation.conv_id,
                        )
                        raise
                    ...
                    refreshed = self.conversation_repo.find_by_id(
                        conversation.conv_id, id_column="conv_id"
                    )
                    if not refreshed:
                        raise ValueError(
                            f"Conversation {conversation.conv_id} not found during retry"
                        )
            ...
            if conversation.status == ConversationStatus.PROGRESS.value:
                context = conversation.context or {}
                if "accepted_by" in context and not context["accepted_by"].get(
                    "message_id"
                ):
                    context = context.copy()
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to update accepted_by message_id due to concurrency",
                            conv_id=conversation.conv_id,
                        )
            ...
            logger.warning(
                "Duplicate message in add_message", conv_id=conversation.conv_id
            )
            raise
        except Exception as e:
    ...
    def _force_close(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        "Max retries reached for force_close (or auto_retry=False)",
                        conv_id=conversation.conv_id,
                    )
                    raise
                ...
                # Reload conversation
                logger.warning(
                    "Concurrency conflict in force_close, retrying...",
                    conv_id=conversation.conv_id,
                )
                refreshed = self.conversation_repo.find_by_id(
                    conversation.conv_id, id_column="conv_id"
    ...
    def close_conversation(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        "Max retries reached for close_conversation (or auto_retry=False)",
                        conv_id=conversation.conv_id,
                    )
                    raise
                ...
                # Reload conversation
                logger.warning(
                    "Concurrency conflict in close_conversation, retrying...",
                    conv_id=conversation.conv_id,
                )
                refreshed = self.conversation_repo.find_by_id(
                    conversation.conv_id, id_column="conv_id"
    ...
    def process_expired_conversations(self, limit: int = 100) -> int:
        ...
        for conversation in expired:
            try:
                ...
                logger.info(
                    "Concurrency conflict during expiration, re-evaluating",
                    conv_id=conversation.conv_id,
                )
                try:
                    refreshed = self.conversation_repo.find_by_id(
                    ...
                    if refreshed and refreshed.is_expired():
                        ...
                        logger.info(
                            "Conversation no longer expired after reload",
                            conv_id=conversation.conv_id,
                        )
                except Exception as e:
                    logger.error(
                        "Error recovering from expiration concurrency",
                        conv_id=conversation.conv_id,
                        error=str(e),
                    )
            except Exception as e:
    ...
    def process_idle_conversations(
        self, idle_minutes: Optional[int] = None, limit: int = 100
    ) -> int:
        """
        ...
        for conversation in idle:
            try:
                ...
                logger.info(
                    "Concurrency conflict during idle timeout, re-evaluating",
                    conv_id=conversation.conv_id,
                )
                try:
                    refreshed = self.conversation_repo.find_by_id(
                    ...
                    if refreshed and refreshed.is_idle(idle_minutes):
                        ...
                        logger.info(
                            "Conversation no longer idle after reload",
                            conv_id=conversation.conv_id,
                        )
                except Exception as e:
                    logger.error(
                        "Error recovering from idle concurrency",
                        conv_id=conversation.conv_id,
                        error=str(e),
                    )
            except Exception as e:
    ...
    def extend_expiration(
        ...
        """
        additional_minutes = (
            additional_minutes or settings.conversation.expiration_minutes
        )

        return self.conversation_repo.extend_expiration(
            conversation.conv_id, additional_minutes
        )

    def transfer_conversation(
        self, conversation: Conversation, new_user_id: str, reason: Optional[str] = None
    ) -> Conversation:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        "Max retries reached for transfer_conversation",
                        conv_id=conversation.conv_id,
                    )
                    raise
                logger.warning(
                    "Concurrency conflict in transfer_conversation, retrying...",
                    conv_id=conversation.conv_id,
                )
    ...
    def escalate_conversation(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        "Max retries reached for escalate_conversation",
                        conv_id=conversation.conv_id,
                    )
                    raise
                logger.warning(
                    "Concurrency conflict in escalate_conversation, retrying...",
                    conv_id=conversation.conv_id,
                )
    ...
    def _handle_critical_error(
        self, conversation: Conversation, error: Exception, context: Dict[str, Any]
    ):
