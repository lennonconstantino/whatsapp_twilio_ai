...
class ClosureDetector:
    ...
    def detect_closure_intent(
        ...
        if not is_user:
            logger.debug(
                f"Message owner is {message.message_owner}, not USER. Skipping closure check."
            )
            return {
                "should_close": False,
                "confidence": 0.0,
        ...
        return {
            ...
            "reasons": reasons,
            "suggested_status": (
                ConversationStatus.USER_CLOSED.value if should_close else None
            ),
        }
    ...
    def _analyze_message_pattern(
        self, current_message: Message, recent_messages: List[Message]
    ) -> float:
        """
        ...
        user_messages = [
            m for m in recent_messages if m.message_owner == MessageOwner.USER
        ]
        ...
        current_content = (
            current_message.body or current_message.content or ""
        ).lower()
        if any(word in current_content for word in positive_words):
            score += 0.2

        # Pattern: final message after sequence of responses
        ai_messages = [m for m in recent_messages[-5:] if m.sent_by_ia]
    ...
    def detect_cancellation_in_pending(
