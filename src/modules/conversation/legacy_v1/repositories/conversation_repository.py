...
class ConversationRepository(SupabaseRepository[Conversation]):
    ...
    def __init__(self, client: Client):
        ...
        super().__init__(
            client, "conversations", Conversation, validates_ulid=True
    ...
    def find_active_by_numbers(
        self, owner_id: str, number1: str, number2: str
    ) -> Optional[Conversation]:
        """
    ...
    def update(
        ...
        try:
            ...
            if isinstance(e, ConcurrencyError):
                raise
            logger.error("Error updating conversation", conv_id=id_value, error=str(e))
            raise
    ...
    def update_status(
        ...
        now = datetime.now(timezone.utc)
        data = {"status": status.value, "updated_at": now.isoformat()}
        ...
        try:
            ...
            if isinstance(e, ConcurrencyError):
                raise
            logger.error(
                "Error updating conversation status", conv_id=conv_id, error=str(e)
            )
            raise
        ...
        try:
            ...
            safe_changed_by = (
                initiated_by if initiated_by in valid_changed_by else "system"
            )
    ...
    def find_by_session_key(
        self, owner_id: str, from_number: str, to_number: str, active_only: bool = True
    ) -> Optional[Conversation]:
        ...
        try:
            ...
            if active_only:
                query = query.in_(
                    "status", [s.value for s in ConversationStatus.active_statuses()]
                )
    ...
    def update_context(
        ...
        data = {"context": context}
        return self.update(
            conv_id, data, id_column="conv_id", expected_version=expected_version
        )

    def extend_expiration(
        self, conv_id: str, additional_minutes: int
    ) -> Optional[Conversation]:
        """
    ...
    def cleanup_expired_conversations(
        ...
        if (owner_id and not channel) or (channel and not owner_id):
            raise ValueError(
                "Ambos owner_id e channel devem ser fornecidos juntos ou nenhum dos dois"
            )
        if phone and not channel:
            raise ValueError(
                "Ambos phone e channel devem ser fornecidos juntos ou nenhum dos dois"
            )
            ...
            for item in result.data or []:
                ...
                if current_status in [
                    ...
                    try:
                        ...
                        logger.warning(
                            "Concurrency conflict during cleanup (active), skipping",
                            conv_id=conv.conv_id,
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.warning(
                            "Concurrency conflict during cleanup (idle), skipping",
                            conv_id=conv.conv_id,
                        )
                        continue
    ...
    def close_by_message_policy(
        ...
        closed = self.update_status(
            ...
            initiated_by=str(
                message_owner.value
                if isinstance(message_owner, MessageOwner)
                else message_owner
            ),
