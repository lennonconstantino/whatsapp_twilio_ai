...
class AIResultService:
    ...
    def get_results_by_message(self, msg_id: str, limit: int = 100) -> List[AIResult]:
        """
    ...
    def analyze_feature_performance(
        ...
        successes = sum(1 for r in results if r.result_json.get("status") == "success")
        errors = sum(1 for r in results if r.result_json.get("status") == "error")
    ...
    def delete_old_results(self, days: int = 90, limit: int = 1000) -> int:
        """
