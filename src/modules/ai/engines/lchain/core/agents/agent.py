    ...
    def run(self, body: str, context: str = None):
        ...
        while i < self.max_steps:
            ...
            if step_result.content and str(step_result.content).strip():
                ...
                if step_result.event in {"assistant", "finish"}:
                    content = str(step_result.content)
                    if not assistant_responses or assistant_responses[-1] != content:
        ...
        if assistant_responses:
            ...
            final_content = last_valid_content or (step_result.content if step_result else None)
