    ...
    def validate_and_check_next_directory(path: str) -> Dict[str, any]:
        ...
        try:
            ...
            if next_path.exists() and next_path.is_dir():
                ...
                result["message"] = f"Success! The '{NEXT_DIRECTORY}' directory exists in {path}"
                result["feature"] = NEXT_DIRECTORY
            else:
                result["message"] = f"The '{NEXT_DIRECTORY}' directory was not found in {path}"
...
if __name__ == "__main__":
    # Test 1
    result = PathValidator.validate_and_check_next_directory("src/modules/ai/engines/lchain/feature")
