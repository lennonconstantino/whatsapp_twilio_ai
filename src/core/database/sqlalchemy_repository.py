...
class SQLAlchemyRepository(Generic[T, M]):
    ...
    def create(self, data: Dict[str, Any]) -> Optional[T]:
        ...
        try:
            ...
            logger.error(f"Error creating record in {self.orm_model.__tablename__}", error=str(e))
            raise

    def find_by_id(self, id_value: Any, id_column: str = "id") -> Optional[T]:
        """Find a record by ID."""
        stmt = select(self.orm_model).where(getattr(self.orm_model, id_column) == id_value)
    ...
    def update(self, id_value: Union[int, str], data: Dict[str, Any], id_column: str = "id") -> Optional[T]:
        ...
        try:
            ...
            self.session.rollback()
            logger.error(f"Error updating record in {self.orm_model.__tablename__}", error=str(e))
            raise

    def delete(self, id_value: Union[int, str], id_column: str = "id") -> bool:
        """Delete a record."""
        try:
            stmt = delete(self.orm_model).where(getattr(self.orm_model, id_column) == id_value)
            result = self.session.execute(stmt)
            self.session.commit()
            return result.rowcount > 0
        except Exception as e:
            self.session.rollback()
            logger.error(f"Error deleting record in {self.orm_model.__tablename__}", error=str(e))
            raise
