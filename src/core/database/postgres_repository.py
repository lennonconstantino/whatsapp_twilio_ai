...
"""
from typing import TypeVar, Generic, Optional, List, Dict, Any, Union, Type
...
class PostgresRepository(Generic[T]):
    ...
    def _execute_query(self, query: sql.Composable, params: tuple = None, fetch_one: bool = False, fetch_all: bool = False) -> Any:
    ...
    def find_by(self, filters: Dict[str, Any], limit: int = 100) -> List[T]:
        ...
        where_clause = sql.SQL(" WHERE {}").format(sql.SQL(' AND ').join(conditions)) if conditions else sql.SQL("")
        limit_clause = sql.SQL(" LIMIT %s")
        
        query = sql.SQL("SELECT * FROM {}").format(sql.Identifier(self.table_name)) + where_clause + limit_clause
    ...
    def count(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        query = sql.SQL("SELECT COUNT(*) as count FROM {}").format(sql.Identifier(self.table_name)) + where_clause
    ...
    def query_dynamic(self, select_columns: List[str] = None, filters: List[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        query = sql.SQL("SELECT {} FROM {}").format(select_clause, sql.Identifier(self.table_name))
        ...
        if filters:
            ...
            for f in filters:
                ...
                if op == 'eq':
                    conditions.append(sql.SQL("{} = {}").format(sql.Identifier(col), sql.Placeholder()))
                elif op == 'gt':
                    conditions.append(sql.SQL("{} > {}").format(sql.Identifier(col), sql.Placeholder()))
                elif op == 'lt':
                    conditions.append(sql.SQL("{} < {}").format(sql.Identifier(col), sql.Placeholder()))
