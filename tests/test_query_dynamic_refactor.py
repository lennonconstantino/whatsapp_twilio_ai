...
from src.modules.ai.engines.lchain.feature.finance.tools.query import (
    QueryConfig, WhereStatement, supabase_query_from_config)
...
class TestQueryDynamicRefactor(unittest.TestCase):
    ...
    def test_tool_integration(self):
        ...
        config = QueryConfig(
            ...
            where=[WhereStatement(column="amount", operator="gt", value=100)],
        )
        ...
        self.assertEqual(
            call_args.kwargs["filters"][0]["value"], 100
        )  # Note: Pydantic might keep it as int/str depending on definition

    def test_tool_validation_error(self):
        """Test tool validation for non-existent columns"""
        config = QueryConfig(
            table_name="test_table", select_columns=["non_existent_column"]
        )
