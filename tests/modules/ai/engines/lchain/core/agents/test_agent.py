...
class TestAgent:
    ...
    async def test_run_step_assistant_response(self, agent, mock_llm_model):
    ...
    async def test_run_step_tool_call(self, agent, mock_llm_model, mock_tool):
    ...
    async def test_run_finish_with_report_tool(self, agent, mock_llm_model):
    ...
    async def test_run_max_steps_reached(self, agent, mock_llm_model):
        ...
        assert mock_llm_model.ainvoke.call_count == 2
    ...
    async def test_error_handling_llm_exception(self, agent, mock_llm_model):
        """Test handling of LLM exceptions."""
        mock_llm_model.ainvoke.side_effect = Exception("API Error")
    ...
    async def test_tool_error_recovery(self, agent, mock_llm_model):
        ...
        mock_llm_model.ainvoke.side_effect = [mock_response_tool, mock_response_apology]
        ...
        with patch(
            ...
            step_result = await agent.run_step([], agent.tools)
            assert step_result.event == "error"
    ...
    async def test_multiple_tool_calls_prevention(self, agent, mock_llm_model):
        ...
        mock_llm_model.ainvoke.side_effect = [mock_response_multi, mock_response_single]
    ...
    async def test_run_with_context(self, agent, mock_llm_model):
        """Test run with context injection."""
        mock_llm_model.ainvoke.return_value = AIMessage(content="Ok")

        await agent.run("Hi", context="Context info")
