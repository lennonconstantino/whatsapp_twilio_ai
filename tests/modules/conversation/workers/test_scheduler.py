...
from src.modules.conversation.workers.scheduler import BackgroundScheduler, SchedulerMetrics
...
class TestBackgroundScheduler(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_schedule_tasks_success(self):
        # Setup settings
        with patch('src.modules.conversation.workers.scheduler.settings') as mock_settings:
    ...
    async def test_schedule_tasks_idle_disabled(self):
        with patch('src.modules.conversation.workers.scheduler.settings') as mock_settings:
            ...
            self.assertEqual(self.mock_queue_service.enqueue.call_args[0][0], "process_expired_conversations")
