...
class TestUserService(unittest.TestCase):
    ...
    def test_create_user_success(self):
        ...
        expected_user = User(
            user_id=self.user_id, owner_id=self.owner_id, phone=dto.phone
        )
    ...
    def test_create_user_phone_exists(self):
        ...
        existing_user = User(
            user_id=generate_ulid(), owner_id=self.owner_id, phone=dto.phone
        )
        ...
        self.assertIn(
            f"User with phone '{dto.phone}' already exists", str(context.exception)
        )
    ...
    def test_get_active_users_by_owner(self):
        expected_users = [
            User(user_id=self.user_id, owner_id=self.owner_id, is_active=True)
        ]
    ...
    def test_get_user_by_auth_id(self):
        auth_id = "auth_123"
        expected_user = User(
            user_id=self.user_id, owner_id=self.owner_id, auth_id=auth_id
        )
    ...
    def test_update_user(self):
        ...
        expected_user = User(
            user_id=self.user_id, owner_id=self.owner_id, first_name="Updated"
        )
        ...
        self.mock_repository.update.assert_called_with(
            self.user_id, data, id_column="user_id"
        )
        self.assertEqual(result, expected_user)
    ...
    def test_deactivate_user(self):
        expected_user = User(
            user_id=self.user_id, owner_id=self.owner_id, is_active=False
        )
    ...
    def test_activate_user(self):
        expected_user = User(
            user_id=self.user_id, owner_id=self.owner_id, is_active=True
        )
