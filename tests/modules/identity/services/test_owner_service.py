...
class TestOwnerService(unittest.TestCase):
    ...
    def test_create_owner_email_exists(self):
        ...
        existing_owner = Owner(
            owner_id=generate_ulid(), name="Existing", email=dto.email
        )
        ...
        self.assertIn(
            f"Owner with email '{dto.email}' already exists", str(context.exception)
        )
    ...
    def test_get_owner_by_id(self):
        expected_owner = Owner(
            owner_id=self.owner_id, name="Test Owner", email="test@example.com"
        )
    ...
    def test_delete_owner(self):
        ...
        self.mock_repository.delete.assert_called_with(
            self.owner_id, id_column="owner_id"
        )
