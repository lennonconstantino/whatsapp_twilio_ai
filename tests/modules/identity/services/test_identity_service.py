...
def test_register_organization_success(identity_service, mock_services, owner_data, admin_user_data, mock_owner, mock_user):
    # Setup mocks
    ...
    # Execute
    result_owner, result_user = identity_service.register_organization(owner_data, admin_user_data)
...
def test_register_organization_owner_creation_failed(identity_service, mock_services, owner_data, admin_user_data):
...
def test_register_organization_user_creation_failed_rollback(identity_service, mock_services, owner_data, admin_user_data, mock_owner):
    mock_services["owner_service"].create_owner.return_value = mock_owner
    mock_services["user_service"].create_user.side_effect = Exception("User creation error")
    ...
    # Verify rollback
    mock_services["owner_service"].delete_owner.assert_called_once_with(mock_owner.owner_id)

def test_register_organization_rollback_failed(identity_service, mock_services, owner_data, admin_user_data, mock_owner):
    mock_services["owner_service"].create_owner.return_value = mock_owner
    mock_services["user_service"].create_user.side_effect = Exception("User creation error")
    mock_services["owner_service"].delete_owner.side_effect = Exception("Rollback error")
...
def test_register_organization_with_features(identity_service, mock_services, owner_data, admin_user_data, mock_owner, mock_user):
    mock_services["owner_service"].create_owner.return_value = mock_owner
    mock_services["user_service"].create_user.return_value = mock_user
    mock_services["plan_service"].plan_repository.find_by_name.return_value = None # No free plan
    
    initial_features = ["feature1", "feature2"]
    
    identity_service.register_organization(owner_data, admin_user_data, initial_features=initial_features)
...
def test_register_organization_feature_creation_error(identity_service, mock_services, owner_data, admin_user_data, mock_owner, mock_user):
    mock_services["owner_service"].create_owner.return_value = mock_owner
    mock_services["user_service"].create_user.return_value = mock_user
    
    mock_services["feature_service"].create_feature.side_effect = [Exception("Error1"), None]
    
    identity_service.register_organization(owner_data, admin_user_data, initial_features=["feat1", "feat2"])
    
    # Should continue despite error
    assert mock_services["feature_service"].create_feature.call_count == 2

def test_register_organization_default_subscription_error(identity_service, mock_services, owner_data, admin_user_data, mock_owner, mock_user):
    mock_services["owner_service"].create_owner.return_value = mock_owner
    mock_services["user_service"].create_user.return_value = mock_user
    ...
    mock_services["subscription_service"].create_subscription.side_effect = Exception("Sub error")
...
def test_get_consolidated_features(identity_service, mock_services):
    ...
    mock_services["subscription_service"].get_active_subscription.return_value = mock_sub
...
def test_get_consolidated_features_errors(identity_service, mock_services):
    ...
    mock_services["subscription_service"].get_active_subscription.side_effect = Exception("Sub error")
    mock_services["feature_service"].get_enabled_features.side_effect = Exception("Feat error")
...
def test_check_feature_access_feature_disabled(identity_service, mock_services, mock_user):
    mock_services["user_service"].get_user_by_id.return_value = mock_user
...
def test_check_feature_access_feature_not_found(identity_service, mock_services, mock_user):
    mock_services["user_service"].get_user_by_id.return_value = mock_user
...
def test_proxied_methods(identity_service, mock_services):
    ...
    mock_services["feature_service"].get_feature_by_name.assert_called_with("owner_1", "feat")
