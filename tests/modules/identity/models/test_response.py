...
class TestResponseModels(unittest.TestCase):
    def setUp(self):
        ...
        self.owner = Owner(
            owner_id=self.owner_id, name="Test Owner", email="owner@test.com"
        )
        ...
        self.subscription_with_plan = SubscriptionWithPlan(
            **self.subscription.model_dump(), plan=self.plan
        )

    def test_owner_with_subscription(self):
        model = OwnerWithSubscription(
            **self.owner.model_dump(), subscription=self.subscription_with_plan
        )
    ...
    def test_user_profile(self):
        ...
        model = UserProfile(
            **self.user.model_dump(), owner=owner_with_sub, permissions={"read": True}
        )
    ...
    def test_register_organization_response(self):
        model = RegisterOrganizationResponse(
            owner=self.owner, admin_user=self.user, subscription=self.subscription
        )
        self.assertEqual(model.owner.owner_id, self.owner_id)
