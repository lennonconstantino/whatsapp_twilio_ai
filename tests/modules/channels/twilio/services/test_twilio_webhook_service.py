...
async def test_process_webhook_local_sender(service, payload, owner_id):
    ...
    with patch("src.modules.channels.twilio.services.twilio_webhook_service.run_in_threadpool", new_callable=AsyncMock) as mock_run:
        mock_run.side_effect = [
            owner_id, # resolve_owner_id
        ]
        
        with patch.object(service, '_process_local_sender', new_callable=AsyncMock) as mock_process_local:
...
async def test_process_webhook_inbound(service, payload, owner_id):
    ...
    with patch("src.modules.channels.twilio.services.twilio_webhook_service.run_in_threadpool", new_callable=AsyncMock) as mock_run:
        ...
        with patch.object(service, '_process_inbound_message', new_callable=AsyncMock) as mock_process_inbound:
...
@pytest.mark.asyncio
async def test_process_inbound_message_success(service, mock_services, payload, owner_id, conv_id, msg_id):
    # Mock conversation
    mock_conv = MagicMock()
    mock_conv.conv_id = conv_id
...
def test_handle_ai_response_success(service, mock_services, payload, owner_id, conv_id, msg_id):
    # Mock dependencies
    mock_user = MagicMock()
    mock_user.model_dump.return_value = {"id": "user_1"}
    mock_services["identity_service"].get_user_by_phone.return_value = mock_user
    
    mock_services["identity_service"].validate_feature_path.return_value = {"feature": "finance"}
...
def test_handle_ai_response_user_not_found(service, mock_services, payload, owner_id, conv_id, msg_id):
...
def test_handle_ai_response_error(service, mock_services, payload, owner_id, conv_id, msg_id):
    mock_services["identity_service"].get_user_by_phone.side_effect = Exception("DB Error")
...
def test_handle_ai_response_empty_agent_response(service, mock_services, payload, owner_id, conv_id, msg_id):
