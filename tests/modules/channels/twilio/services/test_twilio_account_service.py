...
class TestTwilioAccountService:
    ...
    @pytest.fixture
    def mock_repo(self):
        return AsyncMock(spec=TwilioAccountRepository)
    ...
    async def test_resolve_account_by_sid_success(self, service, mock_repo, mock_account):
        """Test resolving account directly by Account SID."""
        mock_repo.find_by_account_sid.return_value = mock_account
        
        result = await service.resolve_account(to_number=None, account_sid="AC12345")
        
        assert result == mock_account
        mock_repo.find_by_account_sid.assert_called_once_with("AC12345")
        mock_repo.find_by_phone_number.assert_not_called()

    async def test_resolve_account_by_phone_success(self, service, mock_repo, mock_account):
        ...
        result = await service.resolve_account(to_number="whatsapp:+1234567890", account_sid=None)
        
        assert result == mock_account
        mock_repo.find_by_phone_number.assert_called_once_with("+1234567890")
        mock_repo.find_by_account_sid.assert_not_called()

    async def test_resolve_account_by_phone_no_prefix(self, service, mock_repo, mock_account):
        """Test resolving account by phone number without prefix."""
        mock_repo.find_by_phone_number.return_value = mock_account
        
        result = await service.resolve_account(to_number="+1234567890", account_sid=None)
        
        assert result == mock_account
        mock_repo.find_by_phone_number.assert_called_once_with("+1234567890")

    async def test_resolve_account_priority_sid(self, service, mock_repo, mock_account):
        """Test that Account SID takes precedence over phone number."""
        mock_repo.find_by_account_sid.return_value = mock_account
        
        result = await service.resolve_account(to_number="whatsapp:+9999999999", account_sid="AC12345")
        
        assert result == mock_account
        mock_repo.find_by_account_sid.assert_called_once()
        mock_repo.find_by_phone_number.assert_not_called()

    async def test_resolve_account_not_found(self, service, mock_repo):
        ...
        with patch("src.modules.channels.twilio.services.twilio_account_service.settings") as mock_settings:
            ...
            result = await service.resolve_account(to_number="whatsapp:+1234567890", account_sid="AC_UNKNOWN")
            
            assert result is None
            mock_repo.find_by_account_sid.assert_called_once_with("AC_UNKNOWN")
            mock_repo.find_by_phone_number.assert_called_once_with("+1234567890")

    async def test_resolve_account_fallback_dev(self, service, mock_repo, mock_account):
    ...
    async def test_resolve_account_no_fallback_prod(self, service, mock_repo):
        ...
        with patch("src.modules.channels.twilio.services.twilio_account_service.settings") as mock_settings:
            ...
            result = await service.resolve_account(to_number=None, account_sid=None)
