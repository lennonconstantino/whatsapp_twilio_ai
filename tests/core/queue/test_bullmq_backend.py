...
def mock_bullmq_worker():
    with patch("src.core.queue.backends.bullmq.Worker") as MockWorker:
        mock_instance = MagicMock() # Worker constructor is synchronous usually, but let's check. 
...
@pytest.mark.asyncio
async def test_bullmq_backend_start_consuming(mock_bullmq_queue, mock_bullmq_worker, mock_aioredis):
    backend = BullMQBackend("redis://localhost:6379")
